...
const rulesInput = z.object({ situacaoAtiva: z.number().int().min(0).max(20).optional(), situacaoInativa: z.number().int().min(0).max(20).optional(), capitalGrande: z.number().int().min(0).max(20).optional(), capitalMedia: z.number().int().min(0).max(20).optional(), capitalPequena: z.number().int().min(0).max(20).optional(), capitalGrandeMin: z.number().int().min(0).max(1_000_000_000).optional(), capitalMediaMin: z.number().int().min(0).max(1_000_000_000).optional(), capitalSemInformacao: z.number().int().min(0).max(20).optional(), cnaeA: z.number().int().min(0).max(20).optional(), cnaeB: z.number().int().min(0).max(20).optional(), cnaeSemClassificacao: z.number().int().min(0).max(20).optional(), geoProximo: z.number().int().min(0).max(20).optional(), geoSecundario: z.number().int().min(0).max(20).optional(), geoDistante: z.number().int().min(0).max(20).optional(), geoProximoKm: z.number().int().min(1).max(10_000).optional(), geoSecundarioKm: z.number().int().min(1).max(10_000).optional(), dddMesmo: z.number().int().min(0).max(20).optional(), dddEstado: z.number().int().min(0).max(20).optional(), dddOutro: z.number().int().min(0).max(20).optional(), minimoExterno: z.number().int().min(1).max(100).optional() });
...
export const appRouter = router({
  ...
  controlPanel: router({
    ...
    updateParameter: adminProcedure.input(z.object({ key: z.string().min(1).max(64), value: z.number().int().min(0).max(1_000_000_000) })).mutation(({ input }) => updateScoringParameter(input.key, input.value)),
