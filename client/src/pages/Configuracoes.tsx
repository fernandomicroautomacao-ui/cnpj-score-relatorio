...
export default function Configuracoes() {
  ...
  async function netlifyControl(action?: string, payload: Record<string, unknown> = {}) {
    const response = await fetch("/api/control", action ? { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ action, ...payload }) } : undefined);
  ...
  useEffect(() => {
    if (useNetlify) { void netlifyControl().then(applyConfig).catch(error => toast.error(error instanceof Error ? error.message : "Falha ao carregar configurações.")); return; }
    if (config.data) applyConfig(config.data);
  }, [useNetlify, config.data]);
  ...
  const updateLocalHub = (hub: Hub, key: keyof Hub, value: string | number) => {
    const safeValue = key === "minimumScore" ? Math.max(1, Number(value) || 1) : value;
    setHubs(current => current.map(item => item === hub ? { ...item, [key]: safeValue } : item));
  };
  ...
  async function restoreAll() {
    try {
      if (useNetlify) applyConfig(await netlifyControl("restoreDefaults"));
      else { await restoreDefaults.mutateAsync(); await utils.controlPanel.list.invalidate(); }
  ...
  return <div className="min-h-screen bg-[#f5f7f8] text-[#102a43]">
    <header className="sticky top-0 z-20 border-b border-[#d9e4ea] bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-[1440px] flex-wrap items-center justify-between gap-4 px-6 py-4"><div className="flex items-center gap-3"><div className="brand-mark"><Settings2 size={19}/></div><div><p className="text-[11px] font-bold uppercase tracking-[.18em] text-[#5f7d8d]">Administração comercial</p><h1 className="text-lg font-bold">Configurações de roteamento</h1></div></div><div className="flex gap-3"><Button onClick={saveAll} disabled={saving} className="h-10 rounded-xl bg-[#0f766e] text-white hover:bg-[#0b6059]"><Save size={16}/>{saving ? "Salvando…" : "Salvar configurações"}</Button><Link href="/" className="inline-flex h-10 items-center gap-2 rounded-xl border border-[#cbdde2] bg-white px-4 text-sm font-medium hover:bg-[#f5fbfa]"><ArrowLeft size={16}/>Voltar à consulta</Link></div></div></header>
    <main className="mx-auto max-w-[1440px] px-6 py-8"><div className="mb-8 max-w-3xl"><Badge className="bg-[#e8f5f2] text-[#0f766e]">Painel persistente</Badge><h2 className="mt-3 text-4xl font-bold tracking-[-.04em]">Hubs, pesos e limites<br/><span className="text-[#0f766e]">em um único lugar.</span></h2><p className="mt-4 max-w-2xl leading-7 text-[#5f7281]">Altere hubs, pesos e limites e use <b>Salvar configurações</b>. Pesos podem ser positivos ou negativos para premiar ou penalizar perfis comerciais.</p></div>
      <Card className="rounded-2xl border-[#d9e4ea] bg-white"><CardHeader className="border-b border-[#edf2f4]"><CardTitle className="flex items-center gap-2"><Building2 size={18} className="text-[#0f766e]"/>Hubs de atendimento</CardTitle><p className="text-sm font-normal text-[#718596]">Marília e Ribeirão Preto são predefinidos e podem ser editados, mas não removidos.</p></CardHeader><CardContent className="space-y-5 p-5"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><HubField label="Nome" value={newHub.nome} onChange={value => setNewHub({ ...newHub, nome:value })}/><HubField label="Cidade" value={newHub.cidade} onChange={value => setNewHub({ ...newHub, cidade:value })}/><HubField label="UF" value={newHub.uf} onChange={value => setNewHub({ ...newHub, uf:value })}/><HubField label="DDD" value={newHub.ddd} onChange={value => setNewHub({ ...newHub, ddd:value.replace(/\D/g, "").slice(0, 2) })}/><HubField label="Latitude" type="number" value={newHub.lat || ""} onChange={value => setNewHub({ ...newHub, lat:Number(value) })}/><HubField label="Longitude" type="number" value={newHub.lon || ""} onChange={value => setNewHub({ ...newHub, lon:Number(value) })}/><HubField label="Pontuação mínima" type="number" value={newHub.minimumScore} onChange={value => setNewHub({ ...newHub, minimumScore:Number(value) })}/><div className="flex items-end"><Button type="button" onClick={addLocalHub} variant="outline" className="h-10 w-full border-[#0f766e] text-[#0f766e]">Adicionar hub</Button></div></div><div className="grid gap-4 md:grid-cols-2">{hubs.map(hub => <HubCard key={`${hub.id ?? "novo"}-${hub.nome}`} hub={hub} onChange={updateLocalHub} onRemove={removeLocalHub}/>)}</div></CardContent></Card>
      <Card className="mt-6 rounded-2xl border-0 bg-[#102a43] text-white"><CardContent className="grid gap-5 p-6 md:grid-cols-[auto_1fr]"><SlidersHorizontal size={22} className="text-[#8fcfc4]"/><div><p className="font-bold">Decisão de roteamento</p><p className="mt-2 text-sm leading-6 text-[#d6e4ea]">O score alcança o mínimo individual do hub; entre os hubs elegíveis, vence a menor distância. Em empate exato, o nome do hub define a ordem estável.</p></div></CardContent></Card>
      <div className="mt-6 grid gap-6 xl:grid-cols-2"><RuleCard title="Pesos de pontuação" items={SCORE_RULES} rules={rules} onChange={updateRule} allowNegative/><RuleCard title="Limiares comerciais" items={LIMIT_RULES} rules={rules} onChange={updateRule}/></div>
      <div className="mt-8 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-[#8ccbc4] bg-[#eaf7f5] p-5"><div><p className="font-semibold text-[#164b46]">Pronto para gravar as alterações?</p><p className="mt-1 text-sm text-[#37605c]">As configurações são salvas diretamente neste painel.</p></div><div className="flex gap-3"><Button type="button" variant="outline" onClick={restoreAll} className="border-[#8ccbc4] bg-white">Restaurar pesos padrão</Button><Button onClick={saveAll} disabled={saving} className="bg-[#0f766e] text-white hover:bg-[#0b6059]"><Save size={16}/>{saving ? "Salvando…" : "Salvar configurações"}</Button></div></div>
    </main>
  </div>;
}

function HubCard({hub,onChange,onRemove}:{hub:Hub;onChange:(hub:Hub,key:keyof Hub,value:string|number)=>void;onRemove:(hub:Hub)=>void}) { return <div className="rounded-xl border border-[#d9e4ea] p-4"><div className="mb-3 flex items-center justify-between"><p className="text-sm font-bold">{hub.isDefault ? "Hub padrão" : "Hub adicional"}</p><Button type="button" variant="ghost" size="sm" disabled={Boolean(hub.isDefault)} onClick={() => onRemove(hub)} className="text-[#a54b4b]">Remover</Button></div><div className="grid gap-3 sm:grid-cols-2"><HubField label="Nome" value={hub.nome} onChange={value => onChange(hub, "nome", value)}/><HubField label="Cidade" value={hub.cidade} onChange={value => onChange(hub, "cidade", value)}/><HubField label="UF" value={hub.uf} onChange={value => onChange(hub, "uf", value.toUpperCase().slice(0, 2))}/><HubField label="DDD" value={hub.ddd} onChange={value => onChange(hub, "ddd", value.replace(/\D/g, "").slice(0, 2))}/><HubField label="Latitude" type="number" value={hub.lat} onChange={value => onChange(hub, "lat", Number(value))}/><HubField label="Longitude" type="number" value={hub.lon} onChange={value => onChange(hub, "lon", Number(value))}/><HubField label="Pontuação mínima" type="number" value={hub.minimumScore} onChange={value => onChange(hub, "minimumScore", Number(value))}/></div></div>; }
