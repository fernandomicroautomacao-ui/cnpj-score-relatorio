...
export default function Home() {
  ...
  const currentGroup = useMemo(() => getCnpjGroup(rawBulk, batchOffset, GROUP_SIZE), [rawBulk, batchOffset]);
  ...
  async function runManualGroup() {
    if (!currentGroup.length) return toast.info("Todos os CNPJs desta lista já foram consultados.");
    setNetlifyBusy(true); setResultsAreBatch(true);
    let successful = 0; let failed = 0;
    for (let index = 0; index < currentGroup.length; index++) {
      const cnpj = currentGroup[index];
      try {
      ...
      if (index < currentGroup.length - 1) await new Promise(resolve => window.setTimeout(resolve, 15_000));
    }
    setNetlifyBusy(false);
    const isDone = batchOffset + currentGroup.length >= rawBulk.length;
    toast.success(isDone ? `Lote concluído: ${successful} sucesso(s) e ${failed} falha(s) neste grupo.` : `Grupo concluído: ${successful} sucesso(s) e ${failed} falha(s). Clique em Pesquisar próximo grupo para continuar.`);
  }
  ...
  function resetManualBatch(content:string) { setBulk(content); setBatchOffset(0); setBatchErrors([]); setResults([]); setResultsAreBatch(true); }
  ...
  const batchProgress = rawBulk.length ? Math.round((batchOffset / rawBulk.length) * 100) : 0;
...
function ManualBatchProgress({processed,total,percentage,errors,isRunning}:{processed:number;total:number;percentage:number;errors:number;isRunning:boolean}) { return <Card className="mt-6 rounded-2xl border-[#a9d7d1] bg-[#eef9f7]"><CardContent className="p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="flex items-center gap-2 font-bold text-[#164b46]"><Clock3 size={17}/>Progresso do lote manual</p><p className="mt-1 text-sm text-[#37605c]">{isRunning ? "Consultando o grupo atual com 15 segundos entre cada CNPJ." : processed >= total ? "Lote concluído." : "O próximo grupo só será consultado quando você clicar no botão."}</p></div><Badge className="bg-white text-[#0f766e]">{processed} de {total}</Badge></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-[#cde6e1]"><div className="h-full rounded-full bg-[#0f766e] transition-[width] duration-300" style={{ width:`${percentage}%` }}/></div><div className="mt-3 flex flex-wrap justify-between gap-2 text-xs text-[#4f7772]"><span>{percentage}% processado · {errors} falha(s)</span><span>{processed >= total ? "Tabela resumida pronta para exportar" : `${Math.ceil((total - processed) / GROUP_SIZE)} grupo(s) restante(s)`}</span></div></CardContent></Card>; }
