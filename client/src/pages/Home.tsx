...
export default function Home() {
  ...
  async function submit() {
    ...
    const payload = { cnpjs:current, overrides:tab === "individual" ? draft ?? undefined : undefined };
  ...
  async function exportPdf() {
    ...
    const payload = { cnpjs:results.map(result => result.cnpj), overrides:draft ?? undefined };
  ...
  return <div className="min-h-screen bg-[#f5f7f8] text-[#102a43]">
    ...
    <main className="mx-auto grid max-w-[1440px] grid-cols-1 gap-7 px-6 py-8 lg:grid-cols-[minmax(0,1fr)_390px]"><section>
      ...
      <p className="mt-5 text-xs leading-5 text-[#718596]">A API Pública do CNPJá permite cinco consultas por minuto por endereço IP. Aguarde um minuto antes de enviar o próximo lote.</p><div className="mt-4 grid gap-4 sm:grid-cols-3"><Metric label="Motor" value="100%" note="sem IA" icon={<Gauge size={17}/>}/><Metric label="Critérios" value="04" note="auditáveis" icon={<CheckCircle2 size={17}/>}/><Metric label="Hubs" value="02" note="comparados" icon={<MapPin size={17}/>}/></div>
