...
type Hub = { nome:string; cidade:string; uf:string; lat:number; lon:number };
...
export default function Home() {
  ...
  const [newHub, setNewHub] = useState<Hub>({ nome:"", cidade:"", uf:"SP", lat:0, lon:0 });
  ...
  function addHub() {
    ...
    setNewHub({ nome:"", cidade:"", uf:"SP", lat:0, lon:0 });
    toast.success("Hub adicionado à comparação.");
  }

  return <div className="min-h-screen bg-[#f5f7f8] text-[#102a43]">
    <header className="sticky top-0 z-10 border-b border-[#d9e4ea] bg-white/90 backdrop-blur"><div className="mx-auto flex max-w-[1440px] items-center justify-between px-6 py-4"><div className="flex items-center gap-3"><div className="brand-mark"><Waypoints size={19}/></div><div><p className="text-[11px] font-bold uppercase tracking-[.18em] text-[#5f7d8d]">Inteligência comercial</p><h1 className="text-lg font-bold tracking-tight">CNPJ Score <span className="font-light text-[#5f7d8d]">/ Roteamento</span></h1></div></div><Badge variant="outline" className="hidden rounded-full border-[#cfe2e4] bg-[#f2fbfa] px-3 py-1 text-[#0f766e] sm:flex">Motor determinístico ativo</Badge></div></header>
    <main className="mx-auto grid max-w-[1440px] grid-cols-1 gap-7 px-6 py-8 lg:grid-cols-[minmax(0,1fr)_390px]"><section>
      <div className="mb-8 max-w-3xl"><p className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-[.2em] text-[#0f766e]"><span className="h-px w-8 bg-[#0f766e]"/>workspace comercial</p><h2 className="text-4xl font-bold leading-[1.06] tracking-[-.04em] sm:text-5xl">Uma decisão de venda,<br/><span className="text-[#0f766e]">sem caixa-preta.</span></h2><p className="mt-5 max-w-xl text-base leading-7 text-[#5f7281]">Consulte dados públicos de CNPJ, revise o cadastro e aplique regras comerciais auditáveis para recomendar o canal mais eficiente.</p></div>
      ...
      <p className="mt-5 text-xs leading-5 text-[#718596]">A API Pública do CNPJá permite cinco consultas por minuto por endereço IP. Aguarde um minuto antes de enviar o próximo lote.</p><div className="mt-4 grid gap-4 sm:grid-cols-3"><Metric label="Motor" value="100%" note="sem IA" icon={<Gauge size={17}/>}/><Metric label="Critérios" value="04" note="auditáveis" icon={<CheckCircle2 size={17}/>}/><Metric label="Hubs" value={String(2 + hubs.length).padStart(2, "0")} note="comparados" icon={<MapPin size={17}/>}/></div>
    ...
    <footer className="mx-auto max-w-[1440px] px-6 pb-8 text-xs text-[#91a3af]">Dados públicos consultados sob demanda · Recomendações determinísticas e revisáveis</footer>
  </div>;
}
...
function HubManager({hubs,newHub,onChange,onAdd,onRemove}:{hubs:Hub[];newHub:Hub;onChange:(hub:Hub)=>void;onAdd:()=>void;onRemove:(name:string)=>void}) { const set=(key:keyof Hub,value:string|number)=>onChange({...newHub,[key]:value}); return <Card className="mt-6 rounded-2xl border-[#cfe2e4] bg-[#f7fbfb]"><CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><MapPin size={17} className="text-[#0f766e]"/>Comparar hubs adicionais</CardTitle><p className="text-xs font-normal leading-5 text-[#718596]">Marília e Ribeirão Preto permanecem fixos. Adicione até 10 hubs por coordenadas para incluí-los na recomendação.</p></CardHeader><CardContent className="space-y-4"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Edit label="Nome do hub" value={newHub.nome} onChange={value=>set("nome",value)}/><Edit label="Cidade" value={newHub.cidade} onChange={value=>set("cidade",value)}/><Edit label="UF" value={newHub.uf} onChange={value=>set("uf",value)}/><Edit label="Latitude" type="number" value={newHub.lat || ""} onChange={value=>set("lat",Number(value))}/><Edit label="Longitude" type="number" value={newHub.lon || ""} onChange={value=>set("lon",Number(value))}/></div><Button type="button" onClick={onAdd} variant="outline" className="h-10 rounded-xl border-[#0f766e] bg-white text-[#0f766e] hover:bg-[#e8f5f2]"><MapPin size={16}/>Adicionar hub</Button>{hubs.length > 0 && <div className="rounded-xl border border-[#d9e4ea] bg-white"><div className="grid grid-cols-[1fr_auto] gap-3 border-b border-[#edf2f4] px-4 py-3 text-xs font-bold uppercase tracking-widest text-[#718596]"><span>Hubs adicionais em comparação</span><span>{hubs.length}/10</span></div>{hubs.map(hub=><div key={hub.nome} className="flex items-center justify-between gap-3 px-4 py-3 text-sm"><span><b>{hub.nome}</b>{hub.cidade ? ` · ${hub.cidade}${hub.uf ? `/${hub.uf}` : ""}` : ""}<small className="ml-2 text-[#718596]">{hub.lat}, {hub.lon}</small></span><Button type="button" variant="ghost" size="sm" onClick={()=>onRemove(hub.nome)} className="text-[#a54b4b] hover:bg-[#fff1f1] hover:text-[#8f3030]">Remover</Button></div>)}</div>}</CardContent></Card>; }
